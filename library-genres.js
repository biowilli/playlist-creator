...
async function createGenrePlaylist(genreName, tracks, token) {
  ...
  const playlistName = `🎵 ${genreName.charAt(0).toUpperCase() + genreName.slice(1).replace('-', ' & ')}`;
  const description = `Auto-generated playlist with ${tracks.length} ${genreName.replace('-', ' & ')} tracks from your library`;
...
async function main() {
  try {
    console.log('🎵 Spotify Library Genre Organizer\n');
    
    // Get access token with library scope
    ...
    const genreMap = categorizeTracksByGenre(savedTracks, artists);
    
    // Filter out genres with too few tracks
    const minTracksPerGenre = 5;
    ...
    for (const [genreName, tracks] of filteredGenres) {
      ...
      createdPlaylists.push({ name: playlist.name, id: playlist.id, trackCount: tracks.length });
    }
    ...
    console.log(`\n🎉 Your library is now organized by genre!`);
